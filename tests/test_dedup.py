...
class TestBasicDedup:
    ...
    @pytest.mark.asyncio
    async def test_message_processed_after_mark(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.mark_processed("chat1", 1, "hello")
        assert store.is_processed("chat1", 1, "hello") is True

    @pytest.mark.asyncio
    async def test_different_message_not_processed(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.mark_processed("chat1", 1, "hello")
...
class TestChatActivityTracking:
    ...
    @pytest.mark.asyncio
    async def test_record_response(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.record_bot_response("chat1", "Hello there!")
    ...
    @pytest.mark.asyncio
    async def test_seconds_since_last(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.record_bot_response("chat1")
    ...
    @pytest.mark.asyncio
    async def test_activity_survives_reload(self, tmp_path):
        path = str(tmp_path / "dedup.json")

        store1 = DeduplicationStore(storage_path=path)
        await store1.record_bot_response("chat1", "Test response")
...
class TestResponseRepetition:
    ...
    @pytest.mark.asyncio
    async def test_exact_match_detected(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.record_bot_response("chat1", "Привет, чем могу помочь?")

        assert store.is_repeating_response("chat1", "Привет, чем могу помочь?") is True

    @pytest.mark.asyncio
    async def test_different_response_ok(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.record_bot_response("chat1", "Привет, чем могу помочь?")
    ...
    @pytest.mark.asyncio
    async def test_highly_similar_detected(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.record_bot_response("chat1", "Корм для собак с аллергией очень важен для здоровья")
    ...
    @pytest.mark.asyncio
    async def test_different_chat_ok(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.record_bot_response("chat1", "Привет!")
...
class TestDedupStats:
    ...
    @pytest.mark.asyncio
    async def test_stats_structure(self, tmp_path):
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))
        await store.mark_processed("chat1", 1, "hello")
        await store.record_bot_response("chat1", "hello back")
...
class TestAsyncConcurrency:
    """Test async lock prevents race conditions."""

    @pytest.mark.asyncio
    async def test_concurrent_mark_processed(self, tmp_path):
        """Test that concurrent writes don't corrupt the database."""
        store = DeduplicationStore(storage_path=str(tmp_path / "dedup.json"))

        async def mark_many(start: int, count: int):
            for i in range(count):
                await store.mark_processed(f"chat_{start}", i, f"message_{i}")
        ...
        # Verify all marks were recorded
        assert store.is_processed("chat_1", 5, "message_5") is True
        assert store.is_processed("chat_2", 7, "message_7") is True
        assert store.is_processed("chat_3", 9, "message_9") is True

        stats = store.get_stats()
        assert stats["total_tracked"] == 30
